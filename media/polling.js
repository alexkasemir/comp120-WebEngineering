...
var Template = function(post) {
    ...
    output = output + "<nav class='navbar navbar-default'><div class='container-fluid'><div class='navbar-header'>";
    output = output + "<a class='navbar-brand' href='#'>" + post.creator + "Purred:</a></div></div></nav>";
    ...
    output = output + "<div class='post-info'><div class='post-footer'>";
    output = output + "<button class='btn btn-success purrButton pull-left' id='" + post.id + "'>Purrrr</button>";
    
    output = output + "<h4 class='postInfo'>" + fixDate(post.time_created) + " | Score: " + post.score + "</h4>";
    output = output + "<button class='btn btn-danger grrButton pull-right' id='" + post.id + "'>Grrrrr!</button></div></div></div>";
    return output;
